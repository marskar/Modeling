...
###################################################################
  ...
  f=1.99; e=0; #change f and e from zero to some value between 0 and 1 to simulate different types and strengths of hypotehtical IFN action
  odeoutput1=lsoda(Y0,timevec,odeequations,c(parms,f,e));
  
  #plot results
  plot(odeoutput1[,1],odeoutput1[,4],type="l",xlab="time (days)",ylab="Virus load",col="green",lwd=2,log="y",xlim=c(0,tmax),ylim=c(1e0,1e7))
